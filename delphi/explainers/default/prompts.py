...
SYSTEM_BESTOFK = """You are a meticulous AI researcher conducting an important investigation into patterns found in language. Your task is to analyze text and provide an explanation that thoroughly encapsulates possible patterns found in it.
Guidelines:
...
def system(cot=False, system_prompt=SYSTEM):
    prompt = ""
...
def system_single_token(system_prompt=SYSTEM_SINGLE_TOKEN):
    return [{"role": "system", "content": system_prompt}]


def system_contrastive(system_prompt=SYSTEM_CONTRASTIVE):
    return [{"role": "system", "content": system_prompt}]
